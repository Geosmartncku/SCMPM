function result = gwre(y,x,east,north,east1,north1,XXXX,info);
...
yhat1=zeros(nm,1);
for iter1=1:nm;
    dx1 = east  - east1(iter1,1);
    dy1 = north - north1(iter1,1);
    d1 = (dx1.*dx1 + dy1.*dy1);
    sd1 = std(sqrt(d1));
    % sort distance to find q nearest neighbors
    ds1 = sort(d1); 
    if dtype == 2, dmax = ds1(q,1); end;
       if dtype == 0,     % Gausian weights 
        wt1 = stdn_pdf(sqrt(d1)/(sd1*bdwt));
       elseif dtype == 1, % exponential weights
        wt1 = exp(-d1/bdwt);
       elseif dtype == 2, % tricube weights
         wt1 = zeros(nm,1);
         nzip1 = find(d1 <= dmax);
         wt1(nzip1,1) = (1-(d1(nzip1,1)/dmax).^3).^3;
       end; % end of if,else 
wt1 = sqrt(wt1);
nzip1 = find(wt1 >= 0.01);
ys1 = y(nzip1,1).*wt1(nzip1,1);
xs1 = matmul(x(nzip1,:),wt1(nzip1,1));
xpxi1 = inv(xs1'*xs1);
b1 = xpxi1*xs1'*ys1;
% compute predicted values
yhatv1 = xs1*b1;
yhat1(iter1,1) = XXXX(iter1,:)*b1;
